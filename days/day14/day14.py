...
def day14_part2(program: List[str]) -> int:
    ...
    for line in program:
        if line.startswith('mask'):
            ...
            powers = [2 ** i for i, c in enumerate(mask) if c == 'X']
            first_address = sum(
                2 ** i
                for i, c in enumerate(mask)
            ...
            for num_powers in range(1, len(powers) + 1):
                for combination in combinations(powers, r=num_powers):
                    mem[first_address + sum(power for power in combination)] = value
    return sum(mem.values())


def main() -> None:
    program = get_program()
    print('Day14, part1 answer:', day14_part1(program))
    print('Day14, part2 answer:', day14_part2(program))
